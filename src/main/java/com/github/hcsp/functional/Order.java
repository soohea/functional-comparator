...
public class Order {
    ...
    public static TreeSet<Order> toTreeSet(List<Order> orders) {
        TreeSet<Order> orderTreeSet = new TreeSet<>(Comparator
                .comparing(Order::isOpen)
                .thenComparing(Order::getAmount)
                .reversed()
                .thenComparing(Order::getOrderTime).thenComparing(Order::getId));
        orderTreeSet.addAll(orders);
        return orderTreeSet;

    }

    static int compareOpen(Order o1, Order o2) {
        if (o1.isOpen() ^ o2.isOpen()) {
            return o1.isOpen() ? -1 : 1;
        }
        return 0;
    }
